const String APLICACAO = "Aplicação";

const String ATIVIDADE = "Atividades";
const String ATIVIDADE_DESCRICAO =
    "Acompanhe todas as suas atividades no Eight.";

const String CONTA = "Conta";

const String PIX_COPIAR = "Copiar chave pix";
const String PIX_CODIGO = "7faa6851-5f31-45c8-831c-fbfafc47d3f9";

const String CRIAR_BOTAO = "Escrever história...";

const String DELETAR = "Deletar conta";
const String DELETAR_DESCRICAO = "Estou saindo do Eight e não pretendo voltar.";

const String DOAR = "Doar";
const String DOAR_DESCRICAO =
    "Aqui é o ponto de encontro entre o desenvolvedor e o usuário.";
const String DOAR_TEXTO =
    'Aqui quem fala é o gênio por trás desse aplicativo sensacional. Surpreendentemente, eu o criei sozinho, sem um exército de robôs ou um assistente superinteligente. Acreditem, foi um verdadeiro desafio! \n\n'
    'Agora, vocês devem estar se perguntando: "Por que diabos eu deveria doar para esse desenvolvedor solitário?". Bem, deixe-me explicar: sua doação é como café para mim, mantendo meus neurônios acordados e motivados. Além disso, me ajuda a continuar adicionando novidades, consertando os bugs e fazendo malabarismos para manter tudo funcionando sem falhas. \n\n'
    'Mas espere, tem mais! Ao fazer sua doação, você ganha um acesso VIP a um clube secreto exclusivo de doadores, onde compartilho dicas e truques ocultos do aplicativo. E se sua doação for realmente generosa, posso até enviar uma foto autografada (pelo app, claro) com uma dedicatória personalizada!\n\n'
    'Agora, vamos combinar, ser o apoiador número um deste desenvolvedor solitário não é só sobre agradecer por todo o esforço e talento investidos, mas também sobre ter o orgulho de fazer parte de algo incrível. Você estará alimentando o sonho de um programador que luta contra as forças do cansaço e da pizza fria para tornar sua experiência no app épica. \n\n'
    'Então, caros usuários fantásticos, vamos unir nossos poderes e mostrar ao mundo o que podemos fazer juntos! Façam suas doações e sejam parte dessa jornada divertida, cheia de risadas e surpresas. \n\n'
    'Agradeço a todos vocês pelo apoio, pela risada compartilhada e por ajudar a manter meu estoque de café sempre abastecido! \n\n'
    'Com gratidão e um sorriso, \n\n'
    'Charles Santos';

const String EDITADO = "Editado";

const String EIGHT = "Eight";

const String ENTRAR_BEM_VINDO = "Seja bem-vindo";
const String ENTRAR_DESCRICAO =
    "Realize seu cadastro ou entre utilizando sua conta do Google. Basta selecionar o botão abaixo para prosseguir.";

const String ERRO_SOLICITACAO =
    "Não foi possível atender sua solicitação, tente novamente mais tarde.";

const String FINALIZAR = "Finalizar";

const String HISTORIA_FIM = "Isso é tudo por enquanto.";

const String MENU = "Menu";
const String NOTIFICACAO = "Notificações";
const String NOTIFICACAO_DESCRICAO = "Monitore e configure as notificações.";

const String PERGUNTAS = "Perguntas frequentes";
const String PERGUNTAS_DESCRICAO =
    "Todas aquelas perguntas que sempre fez em um só lugar.";

const String PERFIL = "Perfil";
const String PERFIL_DESCRICAO = "Veja seu perfil e histórias";

const String POLITICA = "Política de privacidade";
const String POLITICA_DESCRICAO =
    "É o documento que descreve como uma organização coleta, usa e protege as informações pessoais dos seus usuários.";

const String POR = "por Charles Santos • v";

const String SAIR = "Sair";
const String SAIR_DESCRICAO =
    "Estou temporariamente saindo do Eight, mas tenho planos de voltar em breve.";

const String SEM_RESULTADO =
    "Sua pesquisa não encontrou resultado correspondente.";

const String SOBRE = "Sobre o Eight";
const String SOBRE_DESCRICAO = "Onde fornecemos informações sobre o Eight.";

const String TERMO = "Termo de uso";
const String TERMO_DESCRICAO =
    "Onde estabelecemos as regras, direitos e responsabilidades de uso de nosso serviço.";

const String USUARIO_EDITAR = "Editar perfil";
const String USUARIO_EDITAR_DESCRICAO = "Editar perfil";
